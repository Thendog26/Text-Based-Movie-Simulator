...
int main(){
	...
	while (menu == true) {
		...
		if (input == 1) {
			...
			ticketMenu = true; // sets ticket menu to true
		...
		if (input == 2){
			...
			while (candyMenu == true){ // Printed Candy Bar options on menu
				...
				while (!(cin >> input) || (input < 1 && input > 7)) { 	// Verification that the user inputs a valid number into the 
					...
					// Reprints menu if input invalid
					cout << "Welcome to Candy-lation" << endl;
					cout << "Select the number for the option you want" << endl;
					cout << "1.Slushie" << endl; 
					cout << "2.Cola" << endl;
				...
				switch (input) {
					case 1:	// Buying Slushie (Option #1)
						if (user->GetMoney() >= slushie->GetItemValue()){ // If the user can afford the item
					...
					case 2: // Buying Cola (Option #2)
						if (user->GetMoney() >= cola->GetItemValue()){ // If the user can afford the item
					...
					case 3: // Buying Popcorn (Option #3)
						if (user->GetMoney() >= Popcorn->GetItemValue()){		// If the user can afford the item
					...
					case 4: // Buying Choc Top (Option #4)
						if (user->GetMoney() >= choctop->GetItemValue()){ // If the user can afford the item
					...
					case 5: // Buying Chocolate Frogs (Option #5)
						if (user->GetMoney() >= chocolatefrog->GetItemValue()){ // If the user can afford the item
					...
					case 6: // Buying Water (Option #6)
						if (user->GetMoney() >= water->GetItemValue()){ // If the user can afford the item
					...
					case 7: // Leave menu (Option #7)
						candyMenu = false; // sets candy bar menu to false to return to main menu
				}
				cout << "\033[2J\033[1;1H"; // Clears screen
				input = 0;
			}
	...
	while (watch == true) {
		...
		// ######## EATING! #################
		bool check = false; // Checks to see if food has been eaten
		...
		// Eats all food that exists in the player inventory if it exists
		// ## Choc Tops ## //
		...
		if(check == false){
			cout << "...but you have no food :(" << endl;		// If not food was eaten
		}
		else {
			cout << "...All that food was good! :D" << endl;	// If food was eaten
			sleep(2);
		}
		...
		switch (input) {
			case 1:  // Brings user to the candy bar (Option #1)
				...
				while (!(cin >> input) || (input < 1 && input > 7)) { 	// Verification that the user inputs a valid number into the 
					...
					// Reprints menu if user gives invalid input
					cout << "Welcome to Candy-lation" << endl;
					cout << "Select the number for the option you want" << endl;
					cout << "1.Slushie" << endl; 
					cout << "2.Cola" << endl;
